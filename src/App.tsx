...
const INDICES_NOTAS: Record<string, number> = {
  C: 0,
  'C#': 1,
  Db: 1,
  D: 2,
...
function calcularDiagramaAcorde(nombreAcorde: string) {
  ...
  if (t6 >= 1 && t6 <= 8) {
    return {
      baseFret: t6,
      barre: { fret: t6, from: 0, to: 5 },
      frets: [t6, t6 + 2, t6 + 2, t6 + 1, t6, t6],
    };
  }
  let t5 = (semitonoRaiz - 9 + 12) % 12;
  return {
    baseFret: t5,
    barre: { fret: t5, from: 1, to: 5 },
    frets: [-1, t5, t5 + 2, t5 + 2, t5 + 2, t5],
  };
}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H}>
        ...
        {shape.barre && (
          <rect
            ...
            y={
              PAD_TOP +
              (shape.barre.fret - baseFret) * FRET_GAP +
              FRET_GAP / 2 -
              3.5
            }
        ...
        {shape.frets.map((fret: number, i: number) => {
          ...
          return (
            <circle
              key={`d${i}`}
              cx={x}
              cy={PAD_TOP + (fret - baseFret) * FRET_GAP + FRET_GAP / 2}
              r={3.8}
              fill={tema.accent}
            />
          );
  ...
  return (
    ...
    >
      {palabras.map((palabra, pIdx) => {
        if (/^\s+$/.test(palabra))
          return (
            <span key={pIdx} style={{ whiteSpace: 'pre' }}>
              {palabra}
            </span>
          );
...
export default function App() {
  ...
  const t = modoOscuro
    ...
    : {
        ...
        onAccent: '#ffffff',
        shadow: '0 4px 20px rgba(11,17,32,0.06)',
      };
  ...
  const [himnos, setHimnos] = useState(himnosIniciales);
  const [himnoActivo, setHimnoActivo] = useState(himnosIniciales[0]);
  const [recientes, setRecientes] = useState([himnosIniciales[0]]);
  const [favoritos, setFavoritos] = useState([himnosIniciales[0]]);
  ...
  useEffect(() => {
    try {
      ...
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          if (!snapshot.empty) {
            const listaCloud = snapshot.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
            setHimnos(listaCloud);
          }
        },
        (err) => {
  ...
  const seleccionarHimno = (h: any) => {
    ...
    setSemitonos(0);
    setRecientes((prev) =>
      [h, ...prev.filter((item) => item.id !== h.id)].slice(0, 8)
    );
  ...
  const guardarFormulario = async (e: React.FormEvent) => {
    ...
    setHimnos((prev) => {
      ...
      if (existe)
        return prev.map((h) => (String(h.id) === idDoc ? himnoCompleto : h));
      return [himnoCompleto, ...prev];
    });

    try {
      await setDoc(doc(db, 'himnos', idDoc), objetoHimno, { merge: true });
    } catch (err) {
      console.warn('Guardado localmente');
    }
  ...
  const navegarAtras = () => {
    ...
    else if (vistaActual === 'categories' || vistaActual === 'formulario')
      setVistaActual('menu');
  };

  const labelSemitonos =
    semitonos === 0 ? '0' : semitonos > 0 ? `+${semitonos}` : `${semitonos}`;
  ...
  const CATEGORIES = [
    {
      ...
      badge: 'S-',
      count: himnos.filter((h) => h.categoria === 'Suplementarios').length,
      hint: 'Himnario suplementario',
      icon: 'stack',
    },
    {
      key: 'Complementarios',
      ...
      badge: 'C-',
      count: himnos.filter((h) => h.categoria === 'Complementarios').length,
      hint: 'Cantos complementarios',
      icon: 'layers',
    },
    {
      key: 'Himnos',
      title: 'Himnos',
      badge: 'H-',
      count: himnos.filter((h) => h.categoria === 'Himnos').length,
      hint: 'Himnario clásico',
      icon: 'music',
    },
    {
      key: 'Nuevos',
      title: 'Nuevos',
      badge: '✨',
      count: himnos.filter((h) => h.categoria === 'Nuevos').length,
      hint: 'Agregados recientemente',
      icon: 'sparkle',
    },
  ];

  const himnosFiltrados =
    query.trim() === ''
      ? []
      : himnos.filter((h) => {
      ...
      {!ocultarSplash && (
        <div
          style={{
            ...
            zIndex: 9999,
            padding: '24px',
            animation: 'animFadeOut 0.45s ease 1.75s forwards',
          }}
        >
          <LogoOficialCanva />
        </div>
        ...
        >
          <div
            style={{
              ...
              flexDirection: 'column',
              padding:
                'max(16px, env(safe-area-inset-top)) 18px max(24px, env(safe-area-inset-bottom)) 18px',
              gap: 16,
            }}
          >
            <header
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  type="button"
                  className="cn-press"
                  onClick={() => setVistaActual('categories')}
                  style={{
                    width: 38,
                    height: 38,
                    borderRadius: 999,
              ...
              <>
                <section>
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 11, color: t.faint, fontWeight: 600 }}
                    >
                      {himnos.length} cantos
                    </span>
                  </div>
                  <div
                    style={{
                ...
                <section>
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      fontSize: 14,
                      boxShadow: `0 8px 20px rgba(224,122,79,0.25)`,
                      fontFamily: "'Lexend', sans-serif",
                    }}
                  >
                    <span
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 999,
                          background:
                            tab === 'recent' ? t.accentSoft : 'transparent',
                      ...
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 999,
                          background:
                            tab === 'fav' ? t.accentSoft : 'transparent',
                  ...
                  >
                    {listTab.map((s) => (
                      ...
                      >
                        ...
                        >
                          {formatearEtiqueta(s) || s.categoria}
                        </span>
                        <span
                          style={{
                            fontWeight: 600,
          ...
          >
            ...
            {vistaActual === 'categories' ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {himnos
                  .filter((h) => h.categoria === categoriaSel)
                  .map((h) => (
                    <div
                      key={h.id}
        ...
        >
          <div
            ...
            style={{
              ...
              flexDirection: 'column',
              padding:
                'max(16px, env(safe-area-inset-top)) 20px max(40px, env(safe-area-inset-bottom)) 20px',
              gap: 20,
            }}
          >
            <header
            ...
            <div className="header-himno-pdf">
              ...
              >
                ...
                >
                  {himnoActivo.categoria}
                </span>
                <span style={{ fontSize: 12, color: t.muted, fontWeight: 600 }}>
                  {himnoActivo.compas} · {himnoActivo.bpm} BPM
                </span>
              </div>
              <h1
                style={{
              ...
              >
                {formatearEtiqueta(himnoActivo)
                  ? `${formatearEtiqueta(himnoActivo)} · ${himnoActivo.titulo}`
                  : himnoActivo.titulo}
              </h1>
              {himnoActivo.autor && (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <button
                  ...
                  style={{
                    ...
                    color: scrolling ? t.onAccent : t.text,
                    fontWeight: 700,
                    fontSize: 13,
                  }}
                >
                  <Icon name={scrolling ? 'pause' : 'play'} size={14} />{' '}
                  {scrolling ? 'Pausar' : 'Auto'}
                </button>
                {scrolling && (
                  <select
                    value={speed}
            ...
            <div
              className="layout-partitura-pdf"
              style={{ display: 'flex', flexDirection: 'column', gap: 24 }}
            >
              <div
                className="carrusel-acordes cn-scroll"
                style={{
                  display: 'flex',
                  gap: 8,
                  overflowX: 'auto',
              ...
              >
                {acordesDelHimno(himnoActivo.textoChordPro).map((ac) => (
                  <DiagramaAcordeLexend
                    key={ac}
                    name={transposeChord(ac, semitonos)}
                    tema={t}
                  />
                ))}
              </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>
                  {idEditando ? 'Editar Cántico' : 'Nuevo Cántico'}
                </h1>
              </div>
              <button
                type="submit"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                ...
                >
                  Letra y Acordes (Formato ChordPro con [Acorde])
                </label>
                <textarea
                  rows={12}
                  required
